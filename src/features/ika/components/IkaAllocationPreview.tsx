import type { IkaAllocationPreview as IkaAllocationPreviewData } from "../types";
...
export function IkaAllocationPreview({
  preview,
}: Readonly<{
  preview: IkaAllocationPreviewData;
}>) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="text-lg font-semibold text-slate-950">
        Κατανομή ΙΚΑ ανά Έργο
      </h3>
      <div className="mt-4 space-y-3">
        ...
        {preview.projectTotals.length === 0 ? (
          <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">
            Δεν υπάρχουν δεδομένα για κατανομή ΙΚΑ.
          </p>
        ) : null}
      </div>
