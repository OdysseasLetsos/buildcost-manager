...
import { getCompanyOfficeById } from "../services/get-company-offices";
import type { ExpenseActionState } from "../types";
import { officeExpenseBatchInputSchema } from "../validators";

const batchAmountFields = [
  { field: "rentAmount", subtype: "rent", label: "Ενοίκιο" },
  {
    field: "officeUtilitiesAmount",
    subtype: "office_utilities",
    label: "Έξοδα γραφείου",
  },
...
export async function saveOfficeExpenses(
  ...
  const validation = officeExpenseBatchInputSchema.safeParse({
    monthId: formData.get("monthId"),
    officeId: formData.get("officeId"),
    expenseDate: formData.get("expenseDate"),
    rentAmount: formData.get("rentAmount"),
    officeUtilitiesAmount: formData.get("officeUtilitiesAmount"),
  ...
  if (!validation.success) {
    return {
      ok: false,
      message: "Ελέγξτε τα στοιχεία των εξόδων γραφείου.",
  ...
  try {
    ...
    if (!office || !office.active) {
      return { ok: false, message: "Το γραφείο δεν είναι διαθέσιμο." };
    }
    ...
    for (const line of lines) {
      const description = `${line.label} - ${office.name}`;
      ...
      if (existingError) {
        console.error("[expenses:saveOfficeExpenses:lookup] Supabase error", {
          message: existingError.message,
          code: existingError.code,
          details: existingError.details,
          hint: existingError.hint,
        });
        return { ok: false, message: "Δεν ήταν δυνατή η αποθήκευση των εξόδων γραφείου." };
      }
      ...
      if (existingExpense) {
        ...
        if (error) {
          console.error("[expenses:saveOfficeExpenses:update] Supabase error", {
            message: error.message,
            code: error.code,
            details: error.details,
            hint: error.hint,
          });
          return { ok: false, message: "Δεν ήταν δυνατή η ενημέρωση των εξόδων γραφείου." };
        }

        updatedCount += 1;
        await writeAuditLog({
          companyId,
          action: "expense.office.updated",
          entityType: "expense",
          entityId: existingExpense.id,
          metadata: { amount: line.amount, subtype: line.subtype, officeId: input.officeId },
        });
      } else {
        ...
        if (error || !expense) {
          console.error("[expenses:saveOfficeExpenses:create] Supabase error", {
            message: error?.message,
            code: error?.code,
            details: error?.details,
            hint: error?.hint,
          });
          return { ok: false, message: "Δεν ήταν δυνατή η δημιουργία των εξόδων γραφείου." };
        }

        createdCount += 1;
        await writeAuditLog({
          companyId,
          action: "expense.office.created",
          entityType: "expense",
          entityId: expense.id,
          metadata: { amount: line.amount, subtype: line.subtype, officeId: input.officeId },
        });
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "Δεν ήταν δυνατή η αποθήκευση των εξόδων γραφείου.",
    };
  }
}
