...
select p.id, f.id
from public.plans p
join public.features f on f.code in ('payments', 'ika')
...
set
  ...
  updated_at = now()
from public.plans professional_plan
where professional_plan.code = 'professional'
  and (
    c.plan_id is null
    or c.plan_id in (
      select id
...
begin
  ...
  if default_plan_id is null then
    raise exception 'Professional plan is missing.';
  end if;
